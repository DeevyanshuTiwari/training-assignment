  ...
  try {
    ...
    if (!res.ok) {
      ...
      try {
        ...
        errorMessage = errorText; // Backend returned plain text
      }
...
window.loadBookings = async function(eventId, eventName) {
  ...
  bookingsTableBody.innerHTML   = '<tr><td colspan="7" style="text-align:center;padding:20px;">Loading...</td></tr>';
...
if (downloadCsvBtn) {
  downloadCsvBtn.addEventListener('click', () => {
    ...
    // CSV Header
    let csvContent = "Name,Email,Phone,Tickets Booked,Date,Status\n";

    // CSV Rows
    currentBookings.forEach(b => {
      // Wrap text in quotes to handle commas inside text
      const name = `"${b.userName || 'N/A'}"`;
      const email = `"${b.userEmail || ''}"`;
      const phone = `"${b.phone || 'N/A'}"`;
      const tickets = b.seatsBooked;
      const date = `"${formatDate(b.bookingTime)}"`;
      const status = `"${b.bookingStatus || 'CONFIRMED'}"`;
      
      csvContent += `${name},${email},${phone},${tickets},${date},${status}\n`;
    });
    ...
    a.href = url;
    // Format filename like: attendees_Summer_Festival.csv
    const safeName = currentEventName.replace(/[^a-z0-9]/gi, '_').toLowerCase();
    a.download = `attendees_${safeName}.csv`;
