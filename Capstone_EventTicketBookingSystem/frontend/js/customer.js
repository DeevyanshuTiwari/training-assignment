...
/** Initialize the Profile Drawer open/close functionality */
function initProfileDrawer() {
  const userChip = document.querySelector('.c-user-chip');
  const drawer = document.getElementById('profileDrawer');
  const overlay = document.getElementById('profileOverlay');
  const closeBtn = document.getElementById('closeDrawerBtn');
  const drawerLogoutBtn = document.getElementById('drawerLogoutBtn');

  if (!drawer || !overlay || !userChip) return;
  ...
  userChip.addEventListener('click', async () => {
    ...
    await fetchUserProfile(); // Fetch latest data when opened
  });
  ...
  if (profileForm) {
    profileForm.addEventListener('submit', async (e) => {
      ...
      // Basic validation
      if (!name || !email) {
        showAlert('profileAlert', 'error', '⚠️ Name and Email are required.');
        return;
      }

      // UI state: disable button while loading
      ...
      try {
        ...
        // Update navbar visually immediately
        ...
        if (navAvatarEl) navAvatarEl.textContent = name.charAt(0).toUpperCase();
        ...
        showAlert('profileAlert', 'error', '⚠️ Could not update profile.');
      } finally {
  ...
  // Close on X or clicking outside (overlay)
  if (closeBtn) closeBtn.addEventListener('click', closeDrawer);
  if (overlay) overlay.addEventListener('click', closeDrawer);

  // Logout from inside drawer
  if (drawerLogoutBtn) {
...
/** Fetch user data from backend and fill the profile form */
async function fetchUserProfile() {
  try {
    ...
    // We handle the fields your API returns, and fallback to localStorage for smooth UX
    const userName = data.name || localStorage.getItem('userName') || '';
    const userEmail = data.userEmail || data.email || localStorage.getItem('userEmail') || '';
    ...
    // Update the big avatar letter in the drawer
    if (drawerAvatar && userName) {
      drawerAvatar.textContent = userName.charAt(0).toUpperCase();
    }
  } catch (err) {
    showAlert('profileAlert', 'error', '⚠️ Could not load profile data.');
  }
}
